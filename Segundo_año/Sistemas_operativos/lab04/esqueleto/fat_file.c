...
static bool ignore_dentry(const fat_dir_entry disk_dentry) {
    ...
    // Reviso si el dentry es el del archivo fs.log
    bool is_log = strncmp((char *)disk_dentry->base_name, log_name, 8) == 0 && strncmp((char *)disk_dentry->extension, LOG_FILE_EXTENSION, 3) == 0;

    // Si es el de fs.log, le pongo su nombre original
    if (is_log) {

        char original_name[] = LOG_FILE_BASENAME;

        disk_dentry->base_name[0] = original_name[0];
...
ssize_t fat_file_pwrite(fat_file file, const void *buf, size_t size,
    ...
    if (fat_table_is_EOC(file->table, cluster)) {
        size_t bytes_restantes = fat_table_get_cluster_remaining_bytes(file->table, bytes_remaining, offset);

        u32 new_cluster = fat_table_seek_cluster(file->table, file->start_cluster, offset - bytes_restantes);

        cluster = fat_table_next_cluster_or_new_cluster(file->table, new_cluster);
    ...
    while (bytes_remaining > 0 && !fat_table_is_EOC(file->table, cluster)) {
        ...
        if (bytes_remaining > 0) {
            cluster = fat_table_next_cluster_or_new_cluster(file->table, cluster);
            if (errno != 0) {
                break;
            }
        }
...
void fat_file_hide(fat_file file, fat_file parent) {
    ...
    // Pongo el primer byte del dentry en 0xe5
    // Lo marco como pendiente de ser eliminado
    file->dentry->base_name[0] = FAT_FILENAME_DELETED_CHAR;

    // Modifico el campo attribs para indicar que no debe ser modficiado por el FS
...
void fat_file_unlink(fat_file file, fat_file parent) {
    ...
    //u32 new_num_clusters = 0, current_num_clusters = 0; esto de aqui no me interesa

    //empiezo en mi primer cluster e inizializo el proximo cluster a utilizar 
    u32 last_cluster = file->start_cluster, next_cluster = 0;

    //agarro la base de mi file y lo pongo en 0xe5
    file->dentry->base_name[0] = FAT_FILENAME_DELETED_CHAR;
